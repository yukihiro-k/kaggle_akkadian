...
Ensemble MBR inference — v4 MAX-DIVERSITY

Changes from 355by_v3.py:
  1. 5 sampling temperatures (was 3) + 3 samples/temp (was 2) → richer stochastic pool
  2. 3-pass LP generation (1.30 / 1.15 / 1.00) instead of 2-pass
  3. 4th generation variant: no-repeat-ngram beam pass for fluency
  4. Default MBR preset → chrf_heavy (chrF++ is competition metric)
  5. Beams 8→10, MBR pool cap 48→64
  6. All v3 features preserved (N-model, badness scoring, final polish, etc.)
"""
...
class EnsembleMBRConfig:
    ...
    model_paths: List[str] = field(default_factory=lambda: [
        ...
        # Add a 3rd model path here when available, e.g.:
        # "/kaggle/input/your-finetuned-model/final_model",
    ])
    ...
    # --- Sampling (multiple temperatures) ------------------------------
    use_sampling:        bool        = True
    sample_temperatures: List[float] = field(default_factory=lambda: [0.50, 0.65, 0.80, 0.95, 1.10])
    ...
    # --- Multi-pass generation (per model) -----------------------------
    use_multi_pass:         bool        = True
    pass_length_penalties:  List[float] = field(default_factory=lambda: [1.30, 1.15, 1.00])

    # --- No-repeat-ngram pass (extra diversity for fluency) ------------
    use_nrng_pass:          bool  = True
    nrng_size:              int   = 3
    nrng_num_beams:         int   = 12
    nrng_num_cands:         int   = 4

    # --- MBR selection -------------------------------------------------
    mbr_preset:    str   = "chrf_heavy"   # one of: balanced, chrf_heavy, consensus, pure_chrf
    mbr_pool_cap:  int   = 64          # raised to handle massive candidate pools

    # --- Final polishing -----------------------------------------------
    use_final_polish:       bool  = True
    badness_threshold:      float = 2.5   # trigger aggressive cleanup if badness >= this
...
class ModelWrapper:
    ...
    def generate_nrng_candidates(
        ...
        """Generate candidates using no-repeat-ngram constraint for fluency."""
        B   = input_ids.shape[0]
        ctx = _amp_ctx(self.device, self.use_amp)
        nb  = max(nrng_num_beams, nrng_num_cands)
        ...
        with ctx:
            out = self.model.generate(
                ...
                do_sample=False,
                num_beams=nb,
                num_return_sequences=nrng_num_cands,
                max_new_tokens=self.cfg.max_new_tokens,
                length_penalty=length_penalty,
                early_stopping=self.cfg.early_stopping,
                repetition_penalty=self.cfg.repetition_penalty,
                no_repeat_ngram_size=nrng_size,
                use_cache=True,
            )
        texts = self.tokenizer.batch_decode(out, skip_special_tokens=True)
        return [texts[i*nrng_num_cands:(i+1)*nrng_num_cands] for i in range(B)]
...
class EnsembleMBREngine:
    ...
    def _run_one_model(
        ...
        # --- No-repeat-ngram pass (extra diversity) ---
        if self.cfg.use_nrng_pass:
            nrng_label = f"{wrapper.label}/nrng(n={self.cfg.nrng_size})"
            self.logger.info(f"  [{nrng_label}] Starting...")
            dl2 = self._build_dataloader(dataset, wrapper)

            with torch.inference_mode():
                for batch_ids, enc in tqdm(dl2, desc=f"  [{nrng_label}]"):
                    if wrapper.stream is not None:
                        with torch.cuda.stream(wrapper.stream):
                            input_ids = enc.input_ids.to(wrapper.device, non_blocking=True)
                            attn      = enc.attention_mask.to(wrapper.device, non_blocking=True)
                    ...
                    try:
                        nrng_pools = wrapper.generate_nrng_candidates(
                            input_ids, attn,
                            nrng_size=self.cfg.nrng_size,
                            nrng_num_beams=self.cfg.nrng_num_beams,
                            nrng_num_cands=self.cfg.nrng_num_cands,
                            length_penalty=length_penalties[0],
                        )
                        for sid, pool in zip(batch_ids, nrng_pools):
                            sid_str = str(sid)
                            if sid_str not in pools_by_id:
                                pools_by_id[sid_str] = []
                            pools_by_id[sid_str].extend(pool)
                        ...
                        if "out of memory" in str(e).lower():
                            self.logger.error(f"OOM [{nrng_label}] — skipping batch")
                            torch.cuda.empty_cache()
                        else:
                            raise
                    except Exception as e:
                        self.logger.error(f"[{nrng_label}] batch error: {e}")
    ...
    def run(self, test_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if cfg.use_multi_pass:
            lp_passes = list(cfg.pass_length_penalties)
        else:
        ...
        nrng_cands = cfg.nrng_num_cands if cfg.use_nrng_pass else 0
        total_cands = (cands_per_pass * len(lp_passes) + nrng_cands) * n_models

        logger.info("=" * 65)
        logger.info("Ensemble × MBR  |  v4 MAX-DIVERSITY")
        ...
        logger.info(f"  NRNG cands/model  : {nrng_cands}")
...
def print_env(cfg: EnsembleMBRConfig):
    ...
    lp_passes = len(cfg.pass_length_penalties) if cfg.use_multi_pass else 1
    nrng_cands = cfg.nrng_num_cands if cfg.use_nrng_pass else 0
    ...
    print(f"LP passes per model          : {lp_passes}  {cfg.pass_length_penalties if cfg.use_multi_pass else []}")
    print(f"NRNG cands per model         : {nrng_cands}")
    print(f"Total pool ({n_models} models)          : ~{(cands * lp_passes + nrng_cands) * n_models} (before dedup)")
    print()
...
if __name__ == "__main__":
    ...
    cfg_snap = {
        ...
        "use_multi_pass": cfg.use_multi_pass,
        "pass_length_penalties": cfg.pass_length_penalties,
        "use_nrng_pass": cfg.use_nrng_pass,
        "nrng_size": cfg.nrng_size,
        "nrng_num_beams": cfg.nrng_num_beams,
        "nrng_num_cands": cfg.nrng_num_cands,
