...
# ---------------------------
# 8) Main: load test, run 2 passes, fuse, save
# ---------------------------
...
path_a = os.path.join(cfg_a.output_dir, "submission_A.csv")
...
# Fuse
fused = fuse_submissions(sub_a, sub_b, prefer="a", tie_badness_thresh=0.5, w_a=0.50, w_b=0.50)  # EXP5: equal weights

# Final sanity (no empties)
fused["translation"] = fused["translation"].fillna("").map(lambda x: UltraInferenceEngine._final_fix_one(str(x), min_words=base_cfg.min_words_fallback))
