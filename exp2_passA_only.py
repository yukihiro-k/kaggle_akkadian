...
# ---------------------------
# 8) Main: EXP2 – Pass A ONLY (no fusion)
# ---------------------------
...
logger.info(f"PassA ONLY: len_pen={cfg_a.length_penalty}, rep_pen={cfg_a.repetition_penalty}, beams={cfg_a.num_beams}, bs={cfg_a.batch_size}")

# Run inference A
engine_a = UltraInferenceEngine(cfg_a)
sub_a = engine_a.run_inference(test_df, run_tag="A_len115")

# Final sanity (no empties)
sub_a["translation"] = sub_a["translation"].fillna("").map(lambda x: UltraInferenceEngine._final_fix_one(str(x), min_words=base_cfg.min_words_fallback))

out_path = os.path.join(base_cfg.output_dir, "submission.csv")
sub_a.to_csv(out_path, index=False)
logger.info(f"Saved FINAL submission (Pass A only): {out_path}")

# Quick summary
lens = sub_a["translation"].map(lambda x: len(str(x).split()))
logger.info(f"FINAL | rows={len(sub_a)} | len(mean/med/min/max)={lens.mean():.1f}/{lens.median():.1f}/{lens.min()}/{lens.max()}")
print(sub_a.head())
print(sub_a.tail())
