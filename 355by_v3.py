...
Ensemble MBR inference — v3 SCORE-OPTIMISED

Changes from 355by_v2.py:
  1. N-model architecture (model_paths list, not hardcoded A/B)
  2. Dual-pass generation per model (two length_penalty values → 2× candidates)
  3. MBR weight presets (balanced / chrf_heavy / consensus / pure_chrf)
  4. Merged postprocessing: badness scoring + conditional aggressive cleanup
  5. Final output polishing (capitalise, punctuate, fallback text)
  6. All v2 fixes preserved (diverse beam, () in forbidden, unload logic, print)
"""
...
class EnsembleMBRConfig:
    ...
    # --- MBR selection -------------------------------------------------
    mbr_preset:    str   = "balanced"   # one of: balanced, chrf_heavy, consensus, pure_chrf
