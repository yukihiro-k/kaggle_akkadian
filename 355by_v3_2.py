...
Ensemble MBR inference — v3.2 MAJOR IMPROVEMENT

Changes from v3.1:
  1. MBR scoring uses geometric mean (matches competition metric)
  2. Candidate pre-filtering removes garbage before MBR
  3. Lexicon-based proper noun repair (OA_Lexicon_eBL.csv)
  4. Postprocessing: stopped removing PN, preserved ()
  5. Generation: tighter temperatures, more beam candidates
"""
...
class EnsembleMBRConfig:
    ...
    # --- Beam search ---------------------------------------------------
    num_beam_cands:      int   = 6    # v3.2: raised from 4 — beam gives best candidates
    num_beams:           int   = 10   # v3.2: raised from 8 to support 6 return seqs
    ...
    sample_temperatures: List[float] = field(default_factory=lambda: [0.55, 0.70, 0.85])
    num_sample_per_temp: int         = 1    # v3.2: reduced to keep pool manageable
    ...
    dual_pass_length_penalty: float = 1.15   # 2nd pass uses this LP

    # --- MBR selection -------------------------------------------------
    mbr_preset:    str   = "geomean"    # optimised for competition metric (geomean of BLEU + chrF++)
    mbr_pool_cap:  int   = 48          # raised from 32 to handle more candidates
...
class MBRSelector:
    ...
    def _pairwise_score(self, a, b):
        """v3.2: Geometric mean of chrF++ and BLEU (matches competition metric)."""
        chrf_s = self._chrfpp(a, b)
        bleu_s = self._bleu(a, b)
        jacc_s = self._jaccard(a, b)

        # Geometric mean of chrF and BLEU (the competition metric)
        # Add small epsilon to avoid zero multiplication
        geomean = math.sqrt(max(chrf_s, 0.1) * max(bleu_s, 0.1))

        # Blend with jaccard for lexical coverage
        return geomean + self.w_jaccard * jacc_s
